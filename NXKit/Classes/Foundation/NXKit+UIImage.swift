...
public extension UIImage {
    ...
    // 生成渐变色图片
    class func image(colors: [UIColor], size: CGSize, start: CGPoint, end: CGPoint) -> UIImage? {
    ...
    class func image(backgroundColor: UIColor, foregroundColor: UIColor, size: CGSize) -> UIImage? {
 ...
 */
public extension UIImage {
    class func resize(image: UIImage?, size: CGSize, by: String) -> UIImage? {
     ...
     */
    func compressImageToByte(length: Int) -> Data {
        ...
        while data.count > length && data.count != lastDataLength {
            ...
            let ratio = Float(length) / Float(data.count)
            let size = CGSize(width: CGFloat(Int(Float(resultImage.size.width) * sqrtf(ratio))), height: CGFloat(Int(Float(resultImage.size.height) * sqrtf(ratio))))
 ...
 */
public extension UIImage {
    class func avarageColor(image: UIImage?) -> UIColor? {
...
public extension UIImage {
    // 把图像调整为头部朝上的方向
    class func fixedOrientation(image: UIImage?) -> UIImage? {
        guard let image = image else {
            return nil
        }
    ...
    // 返回特定颜色的图片
    class func image(image: UIImage?, color: UIColor, blendMode: CGBlendMode = .destinationIn) -> UIImage? {
        guard let image = image else {
            return nil
        }
        UIGraphicsBeginImageContextWithOptions(image.size, false, image.scale)
