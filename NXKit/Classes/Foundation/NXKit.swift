...
open class NXKit {
    ...
    public static let namespace = NXKit.infoDictionary["CFBundleExecutable"] as? String ?? ""
    public static let name = NXKit.infoDictionary["CFBundleDisplayName"] as? String ?? ""
    public static let version = NXKit.infoDictionary["CFBundleShortVersionString"] as? String ?? ""
    public static let build = NXKit.infoDictionary["CFBundleVersion"] as? String ?? ""
}

public extension NXKit {
    typealias Completion<Value> = (_ value: Value) -> Void
    typealias Event<Event, Value> = (_ event: Event, _ value: Value) -> Void
}
...
public extension NXKit {
    ...
    static let size = CGSize(width: NXKit.width, height: NXKit.height)
    static let scale = max(UIScreen.main.scale, 1.0)
    static let pixel = 1.0 / NXKit.scale // 一个像素的宽度
    ...
    // 判断是不是X屏幕
    static var isXDisplay: Bool {
        return NXKit.safeAreaInsets.bottom > 0.0
    }

    static var toolViewOffset: CGFloat = 49.0
}

// 颜色
public extension NXKit {
    // 主色
    static var primaryColor = NXKit.color(51, 120, 246, 1)
    // 背景色
    static var backgroundColor = NXKit.color(255, 255, 255, 1)
    // view背景色
    static var viewBackgroundColor = NXKit.color(247, 247, 247, 1)
    // 未选中背景色
    static var cellBackgroundColor = NXKit.color(255, 255, 255, 1.0)
    // 选中背景色
    static var cellSelectedBackgroundColor = NXKit.color(234, 234, 234, 0.3)
    // navigationView背景色
    static var barBackgroundColor = NXKit.color(255, 255, 255, 1)
    // navigationView背景色
    static var barForegroundColor = NXKit.color(51, 51, 51, 1)

    // overlay背景色
    static var overlayBackgroundColor = NXKit.color(255, 225, 225, 1)

    // 分割线颜色
    static var separatorColor = NXKit.color(235, 235, 240, 1)
    // 阴影颜色
    static var shadowColor = NXKit.color(56, 79, 134, 1)
    // 深黑
    static var blackColor = NXKit.color(51, 51, 51, 1)
    // 深灰
    static var darkGrayColor = NXKit.color(102, 102, 102, 1)
    // 浅灰
    static var lightGrayColor = NXKit.color(153, 153, 153, 1)
    // 转场前容器视图的Alpha值
    static var transitionInoutBackgroundColor = NXKit.color(0, 0, 0, 0)
    // 转场后容器视图的Alpha值
    static var transitionBackgroundColor = NXKit.color(0, 0, 0, 0.3)

    // 颜色:rgb+alpha, rgb: [0,255],a: [0,1]
    class func color(_ r: CGFloat, _ g: CGFloat, _ b: CGFloat, _ a: CGFloat = 1.0) -> UIColor {
        return UIColor(red: r / 255.0, green: g / 255.0, blue: b / 255.0, alpha: a)
    }

    // 颜色：hex+alpha
    class func color(_ hex: Int, _ a: CGFloat = 1.0) -> UIColor {
        return NXKit.color(CGFloat((hex & 0xFF0000) >> 16), CGFloat((hex & 0xFF00) >> 8), CGFloat(hex & 0xFF), a)
    }

    // 创建浅色/暗黑模式的颜色
    class func color(_ lightColor: UIColor, _ darkColor: UIColor? = nil) -> UIColor {
...
public extension NXKit {
    // 字体
    class func font(_ size: CGFloat, _ weight: UIFont.Weight = UIFont.Weight.regular) -> UIFont {
        return UIFont.systemFont(ofSize: size, weight: weight)
    }

    // 自定义字体
    class func font(_ name: String, _ size: CGFloat) -> UIFont {
...
public extension NXKit {
    // 设备model与产品的映射表
    static var devices: [String: Any] = NXSerialization.file(toDictionary: NXKit.Association.root + "/NXKit.bundle/NXKit.bundle/device.json")

    static let is320x480 = CGSize(width: 320, height: 480)
    // 1x [iPhone,iPhone3GS]
    // 2x [iPhone4,iPhone4S]

    static var is320x568 = CGSize(width: 320, height: 568)
    // 2x [iPhone5,iPhone5c,iPhone5S,iPhoneSE1]

    static var is375x667 = CGSize(width: 375, height: 667)
    // 2x [iPhone6,iPhone6s,iPhone7,iPhone8,iPhoneSE2,iPhoneSE3]

    static var is375x812 = CGSize(width: 375, height: 812)
    // 3x: [iPhone12mini,iPhone13mini],[iPhoneX,iPhoneXs,iPhone11Pro]

    static var is390x844 = CGSize(width: 390, height: 844)
    // 3x: [iPhone12,iPhone12Pro,iPhone13,iPhone13Pro,iPhone14]

    static var is393x852 = CGSize(width: 393, height: 852)
    // 3x: [iPhone14Pro]

    static var is414x736 = CGSize(width: 414, height: 736)
    // 3x: [iPhone6Plus,iPhone6sPlus,iPhone7Plus,iPhone8Plus]

    static var is414x896 = CGSize(width: 414, height: 896)
    // 2x: [iPhoneXr,iPhone11]
    // 3x: [iPhoneXs max,iPhone11ProMax]

    static var is428x926 = CGSize(width: 428, height: 926)
    // 3x: [iPhone12ProMax,iPhone13ProMax,iPhone14Plus]

    static var is430x932 = CGSize(width: 430, height: 932)
    // 3x: [iPhone14ProMax]
}
...
public extension NXKit {
    // 加载获取bundle中图片
    class func image(named name: String, mode: UIImage.RenderingMode = .automatic) -> UIImage? {
    ...
    // 处理图片浏览
    class func previewAssets(assets: [Any], index: Int) {
        NXKit.Imp.previewAssets?(assets, index)
    }

    // 设置图像
    class func image(_ targetView: UIView?, _ url: String, _ state: UIControl.State = UIControl.State.normal) {
...
public extension NXKit {
    // toast
    @discardableResult
    class func showToast(message: String, _ ats: NXKit.Ats = .center, _ superview: UIView? = UIApplication.shared.keyWindow) -> NXHUD.WrappedView? {
    ...
    // 处理loading
    @discardableResult
    class func showLoading(_ message: String, _ ats: NXKit.Ats = .center, _ superview: UIView? = UIApplication.shared.keyWindow) -> NXHUD.WrappedView? {
    ...
    // 处理loading
    class func hideLoading(_ animationView: NXHUD.WrappedView? = nil, superview: UIView? = UIApplication.shared.keyWindow) {
...
// 重定尺寸
public extension NXKit {
    class func resize(size: CGSize, to: CGSize, mode: UIView.ContentMode) -> CGRect {
...
public extension NXKit {
    static var isLoggable: Bool = true
    private static let formatter = DateFormatter()
    class func print(_ items: Any?, _ file: String = #file, _ method: String = #function, _ line: Int = #line) {
    ...
    class func log(_ get: @autoclosure () -> Any?, _ file: String = #file, _ method: String = #function, _ line: Int = #line) {
    ...
    // yyyy-MM-dd HH:mm
    class func descriptionOf(date: Date, format: String) -> String {
...
public extension NXKit {
    ...
    class Association {
        // Framework所在的路径
        public static var root = Bundle(for: NXKit.self).bundlePath
        ...
        public static var openURLs = [String]()

        // 黑名单:在http,https下处理
        public static var blackURLs = [String]()

        // 白名单:在http,https下处理
        public static var whiteURLs = [String]()

        // 拦截的资源的url
        public static var schemes = [String]()

        // js注入脚本
        public static var scripts = [WKUserScript]()

        // alert/action的单元格最大宽度、高度
        public static var size = CGSize(width: min(NXKit.width - 40 * 2, 350), height: 48.0)
    }

    class Imp {
        // 处理图片浏览
        public static var previewAssets: ((_ assets: [Any], _ index: Int) -> Void)?

        // 设置url
        public static var image: ((_ targetView: UIView?, _ url: String, _ state: UIControl.State) -> Void)?

        // encodeURIComponent
        public static var encodeURIComponent: ((_ uri: String) -> (String))?

        // decodeURIComponent
        public static var decodeURIComponent: ((_ uri: String) -> (String))?

        // 授权的回调
        public static var authorization: ((_ type: NXKit.Authorize, _ queue: DispatchQueue, _ isAlertable: Bool, _ completion: ((NXKit.AuthorizeState) -> Void)?) -> Void)?

        // 这里处理网页中回调的js桥接
        public static var webView: ((_ action: String, _ contentView: WKWebView, _ value: [String: Any], _ viewController: NXWebViewController?) -> Bool)?

        // 这里处理单元格高度的通过autoLayout计算的一种回调
        public static var tableView: ((_ tableView: UITableView?, _ value: any NXAnyRepresentable, _ indexPath: IndexPath) -> CGFloat)?

        // 处理toast
        public static var showToast: ((_ message: String, _ ats: NXKit.Ats, _ superview: UIView?) -> NXHUD.WrappedView?)?

        // 处理animation
        public static var showLoading: ((_ message: String, _ ats: NXKit.Ats, _ superview: UIView?) -> NXHUD.WrappedView?)?

        // 处理animation
        public static var hideLoading: ((_ animationView: NXHUD.WrappedView?, _ superview: UIView?) -> Void)?

        // 处理网络请求
        public static var request: ((_ request: NXRequest, _ completion: NXKit.Event<String, NXResponse>?) -> Void)?
    }
}
...
public extension NXKit {
    // request
    class func request(_ request: NXRequest, _ completion: NXKit.Event<String, NXResponse>?) {
...
public extension NXKit {
    // encodeURIComponent
    class func encodeURIComponent(_ uri: String) -> String {
    ...
    // decodeURIComponent
    class func decodeURIComponent(_ uri: String) -> String {
    ...
    // openURL
    class func open(_ url: URL, _ options: [UIApplication.OpenExternalURLOptionsKey: Any], completion: ((Bool) -> Void)? = nil) {
    ...
    // 获取授权/请求授权入口
    class func authorization(_ type: NXKit.Authorize, _ queue: DispatchQueue, _ isAlertable: Bool, _ completion: ((NXKit.AuthorizeState) -> Void)?) {
        NXKit.Imp.authorization?(type, queue, isAlertable, completion)
    }

    // 这里处理网页中回调的js桥接
    @discardableResult
    class func dispose(_ action: String, _ webView: WKWebView, _ value: [String: Any], _ viewController: NXWebViewController?) -> Bool? {
        return NXKit.Imp.webView?(action, webView, value, viewController)
    }

    // 这里处理单元格高度的通过autoLayout计算的一种回调
    class func heightForRow(_ tableView: UITableView?, _ value: any NXAnyRepresentable, _ indexPath: IndexPath) -> CGFloat? {
        return NXKit.Imp.tableView?(tableView, value, indexPath)
    }
}

public extension NXKit {
    class func get(dictionary: [String: Any]?, _ nonnullValue: [String: Any] = [:]) -> [String: Any] {
    ...
    class func get(array: [Any]?, _ nonnullValue: [Any] = []) -> [Any] {
    ...
    class func get(string: String?, _ nonnullValue: String = "") -> String {
    ...
    class func get(int: Int?, _ nonnullValue: Int = 0) -> Int {
        if let __value = int {
            return __value
        }
        return nonnullValue
    ...
    class func get(float: CGFloat?, _ nonnullValue: CGFloat = 0.0) -> CGFloat {
    ...
    class func get(bool: Bool?, _ nonnullValue: Bool = false) -> Bool {
        if let __value = bool {
            return __value
        }
        return nonnullValue
    ...
    class func get<T: Equatable>(value: T?, _ nonnullValue: T) -> T {
...
extension NXKit {
    ...
    open class Separator: NXKit.View {
        ...
        public required init() {
            super.init()
            isHidden = true
            backgroundColor = NXKit.separatorColor
        }
...
public extension NXKit.View {
    class func update(_ metadata: NXKit.Appearance, _ view: UIView) {
    ...
    class func update(_ metadata: NXKit.Attribute, _ view: UIView) {
...
extension NXKit {
    open class Wrappable<Key: NXInitialValue, OldValue: NXInitialValue, Value: NXInitialValue> {
        ...
        open var dispose: ((_ action: String, _ value: Any?, _ completion: NXKit.Event<String, NXKit.Wrappable<Key, OldValue, Value>>?) -> Void)?
    ...
    open class Comparable<MinValue: NXInitialValue, MaxValue: NXInitialValue, Value: NXInitialValue> {
        ...
        open var dispose: ((_ action: String, _ value: Any?, _ completion: NXKit.Event<String, NXKit.Comparable<MinValue, MaxValue, Value>>?) -> Void)?
...
extension Dictionary: NXInitialValue {
    public static var initialValue: [Key: Value] {
        return [Key: Value]()
    }
}

extension Array: NXInitialValue {
    public static var initialValue: [Element] {
        return [Element]()
    }
}
