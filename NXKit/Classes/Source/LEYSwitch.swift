...
//  NXFoundation
//
//  Created by firelonely on 2018/5/10.
...
open class LEYSwitch: LEYControl {
    ...
    open var offTintColor: UIColor = LEYApp.color(0xFFFFFF, 1) {
        didSet {
    ...
    //选中颜色后背景色   默认0xFFFFFF
    open var onTintColor: UIColor = LEYApp.color(0xFFFFFF, 1) {
        didSet {
    ...
    /*
     样式 默认oval
     */
    open var shape: LEYSwitch.Shape = .oval {
        didSet {
    ...
    open override func updateSubviews(_ action:String, _ value:Any?){
        if action == "shape" {
            ...
            if self.on {
                ...
                thumbView.backgroundColor = onThumbTintColor ?? LEYApp.mainColor
                ...
                thumbView.backgroundColor = offThumbTintColor ?? LEYApp.darkGrayColor
            }
        }
    }
