...
open class LEYNavigationController: UINavigationController, UIGestureRecognizerDelegate {
    
    /// 全屏返回手势，默认开启全屏手势返回，如果要禁掉全屏手势返回，请在相应的控制器中执行如下代码即可
    public let panRecognizer = UIPanGestureRecognizer()
    /// 动画相关
    public let ctxs = LEYNavigationController.Associated()
    ...
    override open func pushViewController(_ viewController: UIViewController, animated: Bool) {
        LEYApp.log { return "class=\(viewController.self)"}
    ...
    open func pushViewController(_ viewController: LEYViewController, animated: Bool, completion:(() -> ())?){
    ...
    public func gestureRecognizerShouldBegin(_ gestureRecognizer: UIGestureRecognizer) -> Bool {
        ...
        if self.panRecognizer == gestureRecognizer {
            ...
            if let fromViewController = self.topViewController as? LEYViewController {
                //如过有弹出的自定义视图不支持手势返回
    ...
    open func showSubviewController(_ viewController: LEYViewController, animated: Bool){
        //添加到的父视图控制器:必须栈顶的视图控制器
        guard let to = self.topViewController as? LEYViewController else {return}
        ...
        viewController.ctxs.transitionView = LEYTransitionView(frame: to.view.bounds, owner:viewController)
        viewController.ctxs.transitionView?.backgroundColor = LEYApp.minAlphaOfColor
        ...
        if viewController.ctxs.oriention == .left {
            ...
            UIView.animate(withDuration: animated ? self.ctxs.duration : 0.0,
                           ...
                           animations: {
                            ...
                            viewController.ctxs.transitionView?.backgroundColor = LEYApp.maxAlphaOfColor
            },completion: {(_) in
    ...
    ///   - animated: 是否需要动画
    open func removeSubviewController(_ viewController: LEYViewController, animated: Bool){
        ...
        if viewController.ctxs.oriention == .left {
            ...
            UIView.animate(withDuration: animated ? self.ctxs.duration:0.0,
                ...
                if let vc = self.currentViewController as? LEYViewController {
    ...
    open var currentViewController : UIViewController? {
        ...
        if let showViewController = self.visibleViewController, showViewController != __returnViewController {
            ...
            if let vc = __returnViewController as? LEYViewController {
                if let __vc = vc.ctxs.subviewControllers.last {
                    //先查找自定义加上去的视图控制器
                    return __vc
                }
                else if let __vc = __returnViewController as? LEYToolViewController {
                    //再查找LEYToolViewController中的
...
extension LEYNavigationController {
    open class Associated {
        weak var owner : LEYNavigationController? = nil
...
open class LEYMixedNavigationController<C:UIViewController>: LEYNavigationController {
