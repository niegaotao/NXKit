...
extension LEYItem {
    ...
    open class Wrapped {
        ...
        open var completion : LEYApp.Completion<String, Any?>? = nil  //点击等回调
...
//单元格model基类
open class LEYItem : NSObject {
    public let ctxs = LEYItem.Wrapped()
...
//分组基类
open class LEYSection<Element: LEYItem> : LEYItem {
    open var elements = [Element]()              //分组下所有的单元格对象
    open var header = LEYItem()                  //头部的分割线
    open var footer: LEYItem? = nil              //尾部应该用不到，默认为nil
...
extension LEYSection {
    //tableView 获取重用cell
    public func dequeue(_ tableView: UITableView, _ indexPath : IndexPath) -> (element: LEYItem, cell:LEYTableViewCell)? {
        ...
        if let cell = tableView.dequeueReusableCell(withIdentifier: element.ctxs.reuse, for: indexPath) as? LEYTableViewCell {
            return (element, cell)
        }
        return nil
    }
    ...
    //tableView 获取重用cell
    public func dequeue(_ collectionView: UICollectionView, _ indexPath : IndexPath) -> (element: LEYItem, cell:LEYCollectionViewCell)? {
        guard indexPath.row >= 0 && indexPath.row < self.elements.count else {
            return nil
        }
        let element = self.elements[indexPath.row]
        ...
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: element.ctxs.reuse, for: indexPath) as? LEYCollectionViewCell {
            return (element, cell)
        }
        return nil
    }
...
open class LEYCollection : NSObject {
    open var sections = [LEYSection]()
    
    //通过下标访问一个分组对象
    open subscript(index: Int) -> LEYSection<LEYItem>? {
    ...
    //根据IndexPath访问一个元素
    open subscript(indexPath: IndexPath) -> LEYItem? {
    ...
    //添加一个分组
    open func append(_ newElement: LEYSection<LEYItem>?){
        if let element = newElement {
    ...
    //批量添加分组
    open func append(contentsOf contents: [LEYSection<LEYItem>]?){
        if let contents = contents, contents.count > 0 {
    ...
    @discardableResult
    open func insert(_ element: LEYSection<LEYItem>?, at index:Int) -> Bool {
    ...
    @discardableResult
    open func remove(_ element: LEYSection<LEYItem>?) -> Bool {
        if let element = element {
    ...
    //替换某个分组
    @discardableResult
    open func replace(_ element: LEYSection<LEYItem>?, at index: Int) -> Bool {
        if let element = element, index >= 0 && index < sections.count {
    ...
    //sections中LEYSection对象的个数
    open var count : Int {
        return sections.count
    }
}

extension LEYCollection {
    ...
    public func dequeue(_ tableView:UITableView?, _ index:Int, _ type:LEYCollection.Dequeue.RawValue) -> (element:LEYItem, reusableView:LEYTableReusableView)?{
        guard let __tableView = tableView, index >= 0 && index < self.sections.count else {
            return nil
        }
        if type == LEYCollection.Dequeue.header.rawValue {
            let header = self.sections[index].header
            guard header.ctxs.cls != nil && header.ctxs.reuse.count > 0 else {
                return nil
            }
            if let reusableView = __tableView.dequeueReusableHeaderFooterView(withIdentifier: header.ctxs.reuse) as? LEYTableReusableView {
                return (header, reusableView)
            }
        }
        else if type == LEYCollection.Dequeue.footer.rawValue {
            ...
            if let reusableView = __tableView.dequeueReusableHeaderFooterView(withIdentifier: footer.ctxs.reuse) as? LEYTableReusableView {
    ...
    public func dequeue(_ collectionView: UICollectionView?, _ indexPath:IndexPath, _ type:LEYCollection.Dequeue.RawValue) -> (elelment:LEYItem, reusableView:LEYCollectionReusableView)? {
        guard let __collectionView = collectionView, indexPath.section >= 0 && indexPath.section < self.sections.count else {
            return nil
        }
        if type == LEYCollection.Dequeue.header.rawValue {
            ...
            if let reusableView = __collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionHeader, withReuseIdentifier: element.ctxs.reuse, for: indexPath) as? LEYCollectionReusableView {
            ...
            if let reusableView = __collectionView.dequeueReusableSupplementaryView(ofKind: UICollectionView.elementKindSectionFooter, withReuseIdentifier: element.ctxs.reuse, for: indexPath) as? LEYCollectionReusableView {
                return (element, reusableView)
            }
        }
...
extension LEYCollection {
    ...
    //元素所在的索引
    @discardableResult
    open func indexPath(of element: LEYItem?) -> IndexPath? {
    ...
    //根据indexPath插入一个元素
    @discardableResult
    open func insert(_ element: LEYItem?, at indexPath: IndexPath) -> Bool {
        if let section = self[indexPath.section] {
    ...
    //新增一个分组,并将新增的分组返回
    @discardableResult
    open func addSection(_ cls: AnyClass = LEYTableReusableView.self, _ reuse:String = "LEYTableReusableView", _ h:CGFloat = 10.0) -> LEYSection<LEYItem> {
        let section = LEYSection(cls, reuse, h)
        self.append(section)
        return section
    }
    ...
    @discardableResult
    open func getLastSection(_ cls: AnyClass = LEYTableReusableView.self, _ reuse:String = "LEYTableReusableView", _ h:CGFloat = 10.0) -> LEYSection<LEYItem> {
    ...
    open func addElementToLastSection(_ element: LEYItem?) {
        let section = self.getLastSection()
        section.append(element)
    }
    
    //批量添加单元格到最后一个分组上
    open func addElementsToLastSection(_ elements: [LEYItem]?) {
        let section = self.getLastSection()
