//
//  LEYActionView+LEYFoundation.swift
//  NXFoundation
//
...
extension LEYActionView {
    ...
    @discardableResult
    class public func alert(title: String, subtitle:String, options:[String], completion:LEYApp.Completion<String, Int>?) -> LEYActionView {
        var optionWrappers = [LEYAction]()
        for option in options {
            optionWrappers.append(LEYAction(title:option, value: nil, completion: {(_, __action) in
                __action.ctxs.size = CGSize(width: LEYApp.Overlay.size.width, height: LEYApp.Overlay.size.height)
                __action.asset.isHidden = true
                __action.title.isHidden = false
                if options.count == 2 {
                    __action.title.frame = CGRect(x: 0, y: 0, width: LEYApp.Overlay.size.width*0.5, height: __action.ctxs.h)
                }
                else {
                    __action.title.frame = CGRect(x: 0, y: 0, width: LEYApp.Overlay.size.width, height: __action.ctxs.h)
                }
                __action.title.color = LEYApp.darkBlackColor
                __action.title.font = LEYApp.font(16, true)
        ...
        /*
         ...
         空白20
         */
        let alertView = LEYActionView(frame: UIScreen.main.bounds)
        //header
        var size = subtitle.stringSize(font: LEYApp.font(15, false), size: CGSize(width: LEYApp.Overlay.size.width-40, height: LEYDevice.height*0.6))
        size.height = ceil(max(20.0, size.height*1.25)) + 2.0
        
        alertView.wrapped.header.frame.size = CGSize(width: LEYDevice.width * 0.8, height: 80.0 + size.height)
        alertView.wrapped.header.separator.side = LEYApp.Side.bottom
        alertView.wrapped.header.backgroundColor = LEYApp.backgroundColor
        ...
        alertView.wrapped.header.title.font = LEYApp.font(17, true)
        ...
        alertView.wrapped.header.center.font = LEYApp.font(15, false)
        ...
        alertView.updateSubviews(LEYActionView.Key.Center.alert.rawValue, nil)
    ...
    @discardableResult
    class public func action(titles: [String], completion:LEYApp.Completion<String, Int>?) -> LEYActionView {
        let options = titles.map { (title) -> LEYAction in
            
            return LEYAction(title:title, value: nil, completion: { (_,  __action:LEYAction) in
                __action.asset.isHidden = true
                __action.title.isHidden = false
                __action.subtitle.isHidden = true
            })
        }
        return LEYActionView.action(options: options, completion: completion)
    }
    
    @discardableResult
    class public func action(options: [LEYAction], completion:LEYApp.Completion<String, Int>?) -> LEYActionView {
        return LEYActionView.action(style: LEYActionView.Key.Footer.action.rawValue, options: options, header:(.none, ""), footer: (.components(false, true, false), "取消"), completion: completion)
    }
    
    @discardableResult
    class public func action(options: [LEYAction], header:LEYActionView.HeaderView.Wrapped, footer: LEYActionView.FooterView.Wrapped, completion:LEYApp.Completion<String, Int>?) -> LEYActionView {
        return LEYActionView.action(style: LEYActionView.Key.Footer.action.rawValue, options: options, header:header, footer:footer, completion: completion)
    }
    
    
    @discardableResult
    class public func action(style: LEYActionView.Key.RawValue, options: [LEYAction], header:LEYActionView.HeaderView.Wrapped, footer: LEYActionView.FooterView.Wrapped, completion:LEYApp.Completion<String, Int>?) -> LEYActionView {
        
        let alertView = LEYActionView(frame: UIScreen.main.bounds)
        //header
        alertView.wrapped.header.frame.size = CGSize(width: LEYDevice.width, height: 60)
        alertView.wrapped.header.separator.side = LEYApp.Side.bottom
        if case .components(let lhs, let center, let rhs, let title) = header.style {
            if lhs == false && center == false && rhs == false && title == false {
                ...
                alertView.wrapped.header.rhs.image = UIImage.image(image:LEYApp.image(named:"uiapp_overlay_close_black.png"), color:LEYApp.darkGrayColor)
            ...
            alertView.wrapped.header.frame.size = CGSize(width: LEYDevice.width, height: 0)
            alertView.wrapped.header.isHidden = true
            alertView.wrapped.header.title.isHidden = true
            alertView.wrapped.header.lhs.isHidden = true
            alertView.wrapped.header.center.isHidden = true
        ...
        if case .components(let lhs, let center, let rhs) = footer.style {
            alertView.wrapped.footer.frame.size = CGSize(width: LEYDevice.width, height: 60+LEYDevice.bottomOffset)
            ...
            alertView.wrapped.footer.center.frame = CGRect(x: 0, y: 0, width: LEYDevice.width, height: 60)
            alertView.wrapped.footer.center.font = LEYApp.font(17)
            alertView.wrapped.footer.center.color = LEYApp.mainColor
            alertView.wrapped.footer.rhs.isHidden = !rhs
        }
        else if case .whitespace = footer.style {
            alertView.wrapped.footer.frame.size = CGSize(width: LEYDevice.width, height: 32.0+LEYDevice.bottomOffset)
            alertView.wrapped.footer.title.isHidden = true
            ...
            alertView.wrapped.footer.frame.size = CGSize(width: LEYDevice.width, height: LEYDevice.bottomOffset)
            alertView.wrapped.footer.title.isHidden = true
            alertView.wrapped.footer.lhs.isHidden = true
            alertView.wrapped.footer.center.isHidden = true
            alertView.wrapped.footer.rhs.isHidden = true
...
extension LEYActionView {
    open class HeaderView : LEYHeaderView {
        
        public typealias Wrapped = (style:LEYActionView.HeaderView.Components, title:String)
        ...
        open override func setupSubviews() {
            ...
            titleView.font = LEYApp.font(18, true)
        ...
        open override func updateSubviews(_ action:String, _ value: Any?){
            guard let appearance = value as? LEYActionView.Appearance<LEYActionView.HeaderView> else {
                return
            }
            ...
            if appearance.lhs.isHidden == false {
                ...
                self.lhsView.contentHorizontalAlignment = LEYApp.Attribute.contentHorizontalAlignment(appearance.lhs.textAlignment)
            }
            else {
            ...
            if appearance.rhs.isHidden == false {
                ...
                self.rhsView.contentHorizontalAlignment = LEYApp.Attribute.contentHorizontalAlignment(appearance.rhs.textAlignment)
            }
            else {
                self.rhsView.isHidden = true
            }
            
            if appearance.separator.side == LEYApp.Side.bottom {
...
extension LEYActionView {
    open class CenterView : LEYCView<LEYCollectionView>, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
        private(set) var appearance = LEYActionView.Component()
        open var completion : LEYApp.Completion<LEYAction, Int>? = nil
        ...
        open override func setupSubviews() {
            ...
            self.contentView.register(LEYActionViewCell.self, forCellWithReuseIdentifier: "LEYActionViewCell")
            self.contentView.delaysContentTouches = false
        }
        
        open override func updateSubviews(_ action:String, _ value: Any?){
            guard let appearance = value as? LEYActionView.Appearance<LEYActionView.CenterView> else {
                return
            }
            self.appearance = appearance
            
            appearance.actions.forEach { (option) in
                if let cls = option.ctxs.cls, cls != LEYActionViewCell.self {
        ...
        public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
            let action = self.appearance.actions[indexPath.item]
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: action.ctxs.reuse, for: indexPath) as! LEYActionViewCell
...
extension LEYActionView {
    open class FooterView : LEYFooterView {
        
        public typealias Wrapped = (style:LEYActionView.FooterView.Components, title:String)
        ...
        open override func updateSubviews(_ action:String, _ value: Any?){
            guard let appearance = value as? LEYActionView.Appearance<LEYActionView.FooterView> else {
                return
            }
            ...
            if appearance.center.isHidden == false {
                ...
                self.centerView.contentHorizontalAlignment = LEYApp.Attribute.contentHorizontalAlignment(appearance.center.textAlignment)
            }
            else {
                self.centerView.isHidden = true
            }
            ...
            if appearance.separator.side == LEYApp.Side.top {
