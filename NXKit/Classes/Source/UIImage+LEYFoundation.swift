//
//  UIImage+LEYFoundation.swift
//  NXFoundation
//
//  Created by firelonely on 2018/5/4.
 ...
 */
extension UIImage {
    open class func resize(image: UIImage?, size:CGSize, by:LEYAsset.Resize.RawValue) -> UIImage? {
        guard let _image = image else {
            return nil
        }
        var __size = CGSize(width: _image.size.width*_image.scale, height: _image.size.height*_image.scale)
        __size = LEYAsset.resize(by: by, __size, size, true)
