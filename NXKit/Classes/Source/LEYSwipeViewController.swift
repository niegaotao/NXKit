...
open class LEYSwipeViewController: LEYContainerController, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    open var wrapped = LEYSection<LEYItem>()
    open var collectionView : LEYCollectionView? = nil
    open var swipeView = LEYSwipeView(frame: CGRect(x: 0, y: LEYDevice.topOffset, width: LEYDevice.width, height: 44))
    ...
    override open func viewDidLoad() {
        ...
        self.contentView.frame = CGRect(x: 0, y: LEYDevice.topOffset+44, width: LEYDevice.width, height: self.view.h-(LEYDevice.topOffset+44))
        
        collectionView = LEYCollectionView(frame: self.contentView.bounds)
        ...
        collectionView?.backgroundColor = LEYApp.collectionViewBackgroundColor
    ...
    open func swipeView(swipeView:LEYSwipeView, index: Int, animated: Bool){
    ...
    open func setupSubviews(_ subviewControllers: [LEYViewController], titles:[String], index: Int = 0){
        self.setupSubviews(subviewControllers, elements: titles, index: index)
    }
    
    open func setupSubviews(_ subviewControllers: [LEYViewController], elements:[Any], index: Int = 0){
        //移除历史数据
        ...
        for (idx, vc) in self.subviewControllers.enumerated() {
            LEYApp.log { return "vc:\(vc)"}
            vc.ctxs.isWrapped = true
            vc.ctxs.superviewController = self
            
            let element = LEYSwipeViewController.Element()
            element.ctxs.size = CGSize(width: LEYDevice.width, height: LEYDevice.height-LEYDevice.topOffset)
            element.ctxs.update(LEYSwipeViewController.Cell.self, "LEYSwipeViewControllerCell" + String(idx+1))
            element.owner = vc
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        ...
        return LEYSwipeViewController.Cell()
    }
    
    public func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        if let rs = self.wrapped.dequeue(collectionView, indexPath) {
            if let element = rs.element as? LEYSwipeViewController.Element, let __owner = element.owner {
                cell.contentView.addSubview(__owner.view)
            }
        }
    }
    ...
    open func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let index = Int((p.x + CGFloat(LEYDevice.width) * 0.5) / CGFloat(LEYDevice.width))
...
extension LEYSwipeViewController {
    
    open class Element : LEYItem {
        open var owner : LEYViewController?
    }
