...
open class LEYWebView: WKWebView {
    ...
    open var callbackWithAlert:((_ text:String, _ actions:[String], _  completion: @escaping LEYApp.Completion<String, Int>) -> ())? = nil
    //重定向判断
    open var navigationAction:((_ navigation:WKNavigationAction, _ completion: @escaping LEYApp.Completion<String, WKNavigationActionPolicy>) -> ())? = nil
    //重定向响应
    open var navigationResponse:((_ navigation:WKNavigationResponse, _ completion: @escaping LEYApp.Completion<String, WKNavigationResponsePolicy>) -> ())? = nil
    //被持有的viewcontroller
    open weak var attachedViewController : LEYWebViewController? = nil
    //内部处理代理的类
    open var wrapped : LEYWebView.Wrapped? = nil
    
    public convenience init(frame:CGRect) {
        let __wrapped = LEYWebView.Wrapped()
        
        let __config = WKWebViewConfiguration()
        if #available(iOS 11.0, *) {
            for scheme in LEYApp.Web.schemes {
    ...
    open func setupSubviews(){
        ...
        if let __wrapped = self.wrapped {
            ...
            if LEYApp.Web.names.count > 0 {
                for name in LEYApp.Web.names {
                    self.configuration.userContentController.add(__wrapped, name: name)
                }
            }
            
            //注入js脚本
            if LEYApp.Web.scripts.count > 0 {
                for script in LEYApp.Web.scripts {
        ...
        self.evaluateJavaScript("navigator.userAgent") { (retValue, error) in
            ...
            LEYApp.log { return "navigator.userAgent=\(ua)"}
        }
    }
...
extension LEYWebView {
    open class Wrapped: NSObject,
        ...
        WKURLSchemeHandler {
        //网页容器
        public fileprivate(set) weak var contentView: LEYWebView?
        
        //WKScriptMessageHandler
        public func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage){
            if LEYApp.Web.names.contains(message.name) {
                LEYApp.log { return "message.name=\(message.name),\nmessage.body=\(message.body)"}
                // 依据协定好的数据格式将参数转成dic
                if let value = message.body as? [String:Any] {
                    ...
                    let dicValue = LEYSerialization.JSONString(toDictionary: value, decode: true)
        ...
        public func webView(_ webView: WKWebView, runJavaScriptAlertPanelWithMessage message: String, initiatedByFrame frame: WKFrameInfo, completionHandler: @escaping () -> Void) {
            if let callback = self.contentView?.callbackWithAlert {
                ...
                LEYActionView.alert(title: "温馨提示", subtitle: message, options: ["确定"]) { (action, index) in
                    completionHandler()
                }
            }
        }
        ...
        public func webView(_ webView: WKWebView, runJavaScriptConfirmPanelWithMessage message: String, initiatedByFrame frame: WKFrameInfo, completionHandler: @escaping (Bool) -> Void) {
            if let callback = self.contentView?.callbackWithAlert {
                ...
                LEYActionView.alert(title: "温馨提示", subtitle: message, options: ["取消","确定"]) { (action, index) in
                    completionHandler(index == 1)
                }
            }
        }
        ...
        public func navigationAction(_ navigationAction: WKNavigationAction, _ completion: @escaping ((_ policy:WKNavigationActionPolicy) -> ())){
            ...
            else if LEYApp.URI.scheme == scheme {
                //这种事通过自定义url重定向打开页面的或常用操作的
                self.dispose(scheme:LEYApp.URI.scheme, mapValue: ["action":url.absoluteString])
                completion(WKNavigationActionPolicy.cancel)
            }
            else if LEYApp.Web.openURLs.contains(scheme) {
        ...
        public func dispose(scheme:String, mapValue: [String:Any]){
            ...
            //如果返回true，则LEYURLNavigator自己有能力处理本次操作，反之返回false
            //如果返回false，则回调给具体的业务模块去处理
            if let isDisposed = LEYApp.dispose("action", __contentView, mapValue, self.contentView?.attachedViewController), isDisposed == true {
                //如果能处理
            }
            else {
                //登录？处理需要回调的
        ...
        public func webView(_ webView: WKWebView, start urlSchemeTask: WKURLSchemeTask) {
            LEYApp.dispose("task", webView, ["task":urlSchemeTask,"status":"start"], self.contentView?.attachedViewController)
        }
        
        @available(iOS 11.0, *)
        public func webView(_ webView: WKWebView, stop urlSchemeTask: WKURLSchemeTask) {
            LEYApp.dispose("task", webView, ["task":urlSchemeTask,"status":"stop"], self.contentView?.attachedViewController)
        }
        
        deinit {
            LEYApp.log { return "LEYWebView.Wrapped"}
        }
    }
}
