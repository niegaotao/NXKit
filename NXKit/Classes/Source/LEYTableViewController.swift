//
//  LEYTableViewController.swift
//  NXFoundation
//
//  Created by firelonely on 2018/8/22.
...
open class LEYTableViewController: LEYViewController, UITableViewDelegate, UITableViewDataSource {
    //表视图
    open var tableView: LEYTableView? = nil
    //数据源管理对象
    public let tableWrapper = LEYTableWrapper()
    ...
    override open func viewDidLoad() {
        ...
        //tableView
        self.tableView = LEYTableView(frame: self.contentView.bounds, style: tableWrapper.tableViewStyle)
        self.tableView?.frame = self.contentView.bounds
        self.tableView?.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        self.tableView?.backgroundColor = LEYApp.tableViewBackgroundColor
        self.tableView?.separatorColor = LEYApp.separatorColor
        self.tableView?.delegate = self
        self.tableView?.dataSource = self
        self.tableView?.separatorStyle = LEYApp.Table.separatorStyle
    ...
    open func tableView(_ tableView: UITableView, viewForHeaderInSection index: Int) -> UIView? {
        ...
        if let rs = self.tableWrapper.dequeue(tableView, index, LEYCollection.Dequeue.header.rawValue) {
            rs.reusableView.updateSubviews("update", rs.element)
            return rs.reusableView
        }
        else {
            if let header = self.tableWrapper[index]?.header {
                if let cls = header.ctxs.cls as? LEYTableReusableView.Type {
    ...
    open func tableView(_ tableView: UITableView, viewForFooterInSection index: Int) -> UIView? {
        ...
        if let rs = self.tableWrapper.dequeue(tableView, index, LEYCollection.Dequeue.footer.rawValue) {
            rs.reusableView.updateSubviews("update", rs.element)
            return rs.reusableView
        }
        else {
            if let footer = self.tableWrapper[index]?.footer {
                if let cls = footer.ctxs.cls as? LEYTableReusableView.Type {
