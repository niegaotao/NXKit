...
extension LEYWebViewController {
    open class BackbarWrapperView: LEYLCRView<LEYButton, UIView, LEYButton> {
        open var backBar : LEYButton {return self.lhsView }
        open var separatorView : UIView {return self.centerView }
        open var closeBar : LEYButton {return self.rhsView }
        open var isAutoable = true
        
        override open func setupSubviews() {
            super.setupSubviews()
            self.lhsView.frame = CGRect(x: 0, y: 0, width: 32, height: 44)
            self.lhsView.setImage(LEYApp.image(named:"navi_back_black.png"), for: .normal)
            self.lhsView.imageEdgeInsets = UIEdgeInsets.init(top: 0, left: -4, bottom: 0, right: 4)
            
            self.centerView.frame = CGRect(x: 32, y: 14, width: LEYDevice.pixel, height: 16)
            ...
            self.rhsView.setImage(LEYApp.image(named:"navi_close_black.png"), for: .normal)
...
open class LEYWebViewController: LEYViewController {
    open var url: String = ""
    ...
    open var webView = LEYWebView(frame: CGRect.zero)
    open var progressView = UIProgressView(progressViewStyle: .bar)
    open var backbarView = LEYWebViewController.BackbarWrapperView(frame: CGRect(x: 0, y: 0, width: 65, height: 44))
    ...
    open override func setupSubviews() {
        ...
        self.progressView.progressTintColor = LEYApp.mainColor
    ...
    open func callbackWithCompletion(_ isCompleted: Bool, _ error:Error?){
        LEYApp.log { return "isCompleted = \(isCompleted)"}
        ...
        if isCompleted && self.backbarView.isAutoable {
            if self.webView.canGoBack {
                ...
                self.naviView.titleView.x = 15.0 + 44.0 + 1.0
                self.naviView.titleView.w = self.naviView.w - self.naviView.titleView.x * 2.0
                LEYApp.print("t.f=\(self.naviView.titleView.frame)")
            }
        }
    }
