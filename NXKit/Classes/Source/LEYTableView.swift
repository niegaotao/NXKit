...
open class LEYTableView: UITableView {
    open var backdropView : UIImageView? = nil
    weak open var tableWrapper : LEYTableWrapper?
    ...
    open func setupSubviews(){
        ...
        self.backgroundColor = LEYApp.tableViewBackgroundColor
        self.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        self.separatorColor = LEYApp.separatorColor
        ...
        self.tableHeaderView = UIView(frame: CGRect(x: 0, y: 0, width: LEYDevice.width, height: 0.01))
        self.tableHeaderView?.backgroundColor = UIColor.clear
        
        self.tableFooterView = UIView(frame: CGRect(x: 0, y: 0, width: LEYDevice.width, height: 0.01))
        ...
        if let __tableWrapper = self.tableWrapper {
            if __tableWrapper.sections.isEmpty {
                ...
                if __tableWrapper.placeholderView.wrapped.isHidden == false {
                    ...
                    var size  = CGSize(width: LEYDevice.width, height: 0)
