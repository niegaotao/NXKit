//
//  LEYAssetClipImageViewController.swift
//  LEYKit
//
...
open class LEYAssetClipImageViewController: LEYViewController {
    open var image : UIImage? = nil
    
    open var clips = LEYApp.Wrapped<Int, [LEYAsset.Clip]> { (_, __sender) in
        __sender.index = -1
        __sender.value = []
    }
    public let backgroundView = LEYCView<UIImageView>(frame: CGRect(x: 10, y: 10, width: LEYDevice.width-20, height: LEYDevice.height-LEYDevice.topOffset-LEYDevice.bottomOffset-20))
    public let clipboardView = LEYClipboardView(frame: CGRect.zero)
    public let footerView = LEYCView<UIScrollView>(frame: CGRect(x: 0, y: 0, width: LEYDevice.width, height: 80+LEYDevice.bottomOffset))
    ...
    open override func viewDidLoad() {
        ...
        self.naviView.forwardBar = LEYNaviView.Bar.forward(image: nil, title: "保存", completion: {[weak self] (_) in
    ...
    override open func setupSubviews() {
        self.contentView.backgroundColor = LEYApp.color(0x181818)
        
        self.backgroundView.backgroundColor = LEYApp.color(0x181818)
        ...
        self.footerView.backgroundColor = LEYApp.color(0x181818)
        self.footerView.contentView.backgroundColor = LEYApp.color(0x181818)
        ...
        if let image = self.image, image.size.width > 0 && image.size.height > 0, self.clips.value.count >= 1 {
            ...
            var __background = CGRect(x: 10, y: 10, width: LEYDevice.width-20, height: LEYDevice.height-LEYDevice.topOffset-LEYDevice.bottomOffset-20)
            if self.clips.value.count >= 2 {
                //有多项可选
                __background = CGRect(x: 10, y: 10, width: LEYDevice.width-20, height: LEYDevice.height-LEYDevice.topOffset-LEYDevice.bottomOffset-20-80)
                ...
                if (LEYDevice.width - ctx.size.width) / 2.0 > ctx.origin.x {
                    ctx.origin.x = (LEYDevice.width - ctx.size.width) / 2.0
                }
                ...
                for (index, clip) in self.clips.value.enumerated() {
                    ...
                    componentView.font = LEYApp.font(13)
                    ...
                    if index == self.clips.index {
                        ...
                        componentView.textColor = LEYApp.color(0x181818)
                    }
                    else {
                        componentView.backgroundColor = LEYApp.color(0x3d3d3d)
                ...
                self.footerView.contentView.frame = CGRect(x: 0, y: 0, width: LEYDevice.width, height: 80)
