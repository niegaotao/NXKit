...
open class LEYAssetsViewController: LEYViewController,UICollectionViewDelegate, UICollectionViewDataSource {
    //配置信息
    public let wrapped = LEYAsset.Wrapped()
    //顶部悬停过滤的
    public let filterView = LEYSuspendView<UIButton>(frame: CGRect(x: 0, y: 0, width: LEYDevice.width, height: 40))
    //展示图片
    public let collectionView = LEYCollectionView(frame: CGRect.zero)
    //展示底部切换的按钮
    public let footerView = LEYFooterView(frame: CGRect(x: 0, y: -(60+LEYDevice.bottomOffset), width: LEYDevice.width, height: 60+LEYDevice.bottomOffset))
    ...
    override open func viewDidLoad() {
        ...
        self.naviView.backBar.updateSubviews(LEYApp.image(named: "navi_close_black.png"), nil)
         ...
         */
        LEYApp.authorization(LEYApp.AuthorizeType.album, DispatchQueue.main, true) {[weak self] (state) in
            if state == .authorized, let self = self {
                self.contentView.addSubview(self.animationView!)
                self.animationView?.startAnimating()
                DispatchQueue(label: "serialQueue", attributes: .init(rawValue: 0)).async {
                    LEYAsset.outputAlbums(self.wrapped, completion: { [weak self] (albums) in
                        ...
                        DispatchQueue.main.async {[weak self] in
                            ...
                            if let count = self?.wrapped.albums.count, count > 0  {
                                ...
                                LEYApp.showToast(message: "您的相册没有图片或视频", .center, self?.contentView)
                            }
                        }
                    })
                }
    ...
    open override func setupSubviews(){
        self.collectionView.frame = CGRect(x: 0, y:0 , width: self.contentView.w, height: self.contentView.h-60-LEYDevice.bottomOffset)
        ...
        self.collectionView.wrapped?.itemSize = CGSize(width: (LEYDevice.width-12*2-2*3)/4, height: (LEYDevice.width-12*2-2*3)/4)
        self.collectionView.autoresizingMask = [.flexibleWidth, .flexibleHeight]
        self.collectionView.backgroundColor = LEYApp.backgroundColor
        self.collectionView.delegate = self
        self.collectionView.dataSource = self
        self.collectionView.register(LEYAssetViewCell.self, forCellWithReuseIdentifier: "LEYAssetViewCell")
        self.collectionView.alwaysBounceVertical = true
        self.contentView.addSubview(self.collectionView)
        if #available(iOS 11.0, *) {
            collectionView.contentInsetAdjustmentBehavior = .never
        }
        self.naviView.forwardBar = LEYNaviView.Bar.forward(image: nil, title: "预览(0)", completion: {[weak self] (_) in
            self?.dispose("forward", nil)
        })
        self.naviView.forwardBar?.setTitleColor(LEYApp.mainColor, for: .normal)
        self.naviView.forwardBar?.isHidden = !self.wrapped.outputUIImage
        
        self.filterView.contentView.frame = CGRect(x: 0, y: 0, width: LEYDevice.width, height: 40)
        self.filterView.contentView.titleLabel?.font = LEYApp.font(13, false)
        self.filterView.contentView.contentHorizontalAlignment = .center
        self.filterView.setupSeparator(color: LEYApp.separatorColor, side: .bottom)
        self.filterView.contentView.setupEvents([.touchUpInside]) {[weak self] (e, v) in
            self?.dispose("filter", nil)
        }
        self.filterView.backgroundColor = LEYApp.backgroundColor
        self.contentView.addSubview(self.filterView)
        self.filterView.updateSubviews("", false)
        
        self.footerView.frame = CGRect(x: 0, y: self.contentView.h-60-LEYDevice.bottomOffset, width: self.contentView.w, height: 60+LEYDevice.bottomOffset)
        self.footerView.autoresizingMask = [.flexibleWidth, .flexibleTopMargin]
        self.footerView.backgroundColor = LEYApp.backgroundColor
        self.footerView.lhsView.frame = CGRect(x: 15, y: 12, width: 144, height: 36)
        self.footerView.lhsView.setTitleColor(LEYApp.darkBlackColor, for: .normal)
        self.footerView.lhsView.titleLabel?.font = LEYApp.font(16, false)
        ...
        self.footerView.rhsView.frame = CGRect(x: LEYDevice.width-15-88, y: 12, width: 88, height: 36)
        ...
        self.footerView.rhsView.setBackgroundImage(UIImage.image(color: LEYApp.mainColor), for: .normal)
        self.footerView.rhsView.titleLabel?.font = LEYApp.font(15, false)
    ...
    open override func updateSubviews(_ action:String, _ value: [String:Any]?){
        guard let album = value?["album"] as? LEYAlbum else {
            return
        }
        ...
        if album.isBlockable {
            ...
            attris.setAttributes([NSAttributedString.Key.font:LEYApp.font(13, false)], range: NSRange(location: 0, length: prefix.count+subfix.count))
            attris.setAttributes([NSAttributedString.Key.foregroundColor:LEYApp.darkGrayColor], range: NSRange(location: 0, length: prefix.count))
            attris.setAttributes([NSAttributedString.Key.foregroundColor:LEYApp.mainColor], range: NSRange(location: prefix.count, length: subfix.count))
    ...
    open override func dispose(_ action: String, _ value: Any?, _ completion: LEYApp.Completion<String, Any?>? = nil) {
        if action == "footer.lhs" {
            LEYActionView.action(options: self.wrapped.albums, header: (.components(false, true, true, false), "请选择相册"), footer: (.whitespace, "")) { (_, index) in
            ...
            if self.wrapped.output.assets.count < self.wrapped.output.minOfAssets {
                if self.wrapped.output.image.minOfAssets > 0 {
                    LEYApp.showToast(message: "请至少选择\(self.wrapped.output.image.minOfAssets)张图片", .center, self.contentView)
                }
                else if self.wrapped.output.video.minOfAssets > 0 {
                    LEYApp.showToast(message: "请至少选择\(self.wrapped.output.video.minOfAssets)个视频", .center, self.contentView)
                }
                return
            }
            ...
            if wrapped.outputUIImage {
                ...
                LEYAsset.outputAssets(self.wrapped, completion:{[weak self] (_, assets:[LEYAsset]) in
                    ...
                    if assets.count == 1 && self.wrapped.imageClips.count >= 1, let image = assets.first?.image  {
                        let vc = LEYAssetClipImageViewController()
                        vc.image = image
                        vc.clips.index = 0
                        ...
                        vc.ctxs.completion = { [weak self] (_, outputUIImage) in
                            ...
                            LEYAsset.Wrapped.dispose(self.wrapped, assets: assets)
                        }
                ...
                LEYAsset.Wrapped.dispose(self.wrapped, assets: self.wrapped.output.assets)
            }
        }
        else if action == "forward" {
            if self.wrapped.outputUIImage {
                if self.wrapped.output.assets.count == 0 {
                    LEYApp.showToast(message: "请选择图片或视频后再预览哦", .center, self.contentView)
                    return;
                }
                ...
                LEYAsset.outputAssets(self.wrapped) {[weak self] (assets, outputs) in
                    self?.wrapped.output.isOutputting = false
                    LEYApp.showAssets(type: "UIImage", assets: outputs)
                }
            }
            else{
                LEYApp.showToast(message: "不支持图片预览哦", .center, self.contentView)
                return;
            }
        }
            ...
            var __size = String.size(of: __description, size: CGSize(width: 200, height: 36), font: LEYApp.font(15))
            __size.width = __size.width + 24
            __size.height = 36
            self.footerView.rhsView.frame = CGRect(x: LEYDevice.width-15-__size.width, y: 12, width: __size.width, height: __size.height)
            self.footerView.rhsView.setTitle(__description, for: .normal)
        }
    }
    
    @objc override open func backBarAction() {
        LEYAsset.Wrapped.close(self.wrapped)
    }
    ...
    public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        if asset.mediaType == .image {
            if __wrapped.output.image.maxOfAssets <= 0 {
                LEYApp.showToast(message: "不支持选择图片", .center, self.contentView)
                return
            }
            
            if __wrapped.output.isMixable {
                if __wrapped.output.assets.count >= __wrapped.output.maxOfAssets {
                    LEYApp.showToast(message: "最多只能选择\(__wrapped.output.maxOfAssets)张图片和视频哦", .center, self.contentView)
                    return
                }
            }
            else {
                if __wrapped.output.video.assets.count > 0  {
                    LEYApp.showToast(message: "不支持同时选择图片和视频", .center, self.contentView)
                    return
                }
            }
        
            if __wrapped.output.image.assets.count >= __wrapped.output.image.maxOfAssets {
                LEYApp.showToast(message: "最多只能选择\(__wrapped.output.image.maxOfAssets)张图片哦", .center, self.contentView)
                return
            }
            ...
            if __wrapped.output.video.maxOfAssets <= 0 {
                LEYApp.showToast(message: "不支持选择视频", .center, self.contentView)
                return
            }
            ...
            if __wrapped.output.video.assets.count >= __wrapped.output.video.maxOfAssets {
                LEYApp.showToast(message: "最多只能选择\(__wrapped.output.video.maxOfAssets)个视频哦", .center, self.contentView)
                return
            }
            ...
            if asset.isSelectable {
                ...
                if asset.duration > __wrapped.videoClipsDuration && __wrapped.videoClipsAllowed == false {
                    LEYApp.showToast(message: "暂时只支持\(__wrapped.videoClipsDuration)秒以内的视频", .center, self.contentView)
                    return
                }
                ...
                LEYApp.showToast(message: "暂时只支持MP4格式的视频", .center, self.contentView)
            }
        }
        else if asset.mediaType == .audio {
            LEYApp.showToast(message: "不支持选择音频哦", .center, self.contentView)
        }
    }
    ...
    public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "LEYAssetViewCell", for: indexPath) as! LEYAssetViewCell
...
extension LEYAssetsViewController : UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    
    func openCamera(){
        
        LEYApp.authorization(LEYApp.AuthorizeType.camera, DispatchQueue.main, true) {[weak self] (status) in
    ...
    public func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [UIImagePickerController.InfoKey : Any]) {
        ...
        if let __output = image {
            ...
            LEYAsset.saveImage(image: __output, completion: {[weak self] (state, asset) in
                ...
                if let __asset = asset {
                    ...
                    LEYAsset.Wrapped.dispose(self.wrapped, assets: [leyAsset])
                }
                else {
                    LEYAsset.Wrapped.dispose(self.wrapped, assets: [])
                }
            })
        }
    }
