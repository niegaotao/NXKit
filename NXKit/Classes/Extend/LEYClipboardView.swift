...
open class LEYClipboardView: LEYView {
    open class Line {
    ...
    open class Wrapped : NSObject {
        ...
        //约束宽高
        open var clip = LEYAsset.Clip(isResizable: false, width: 1.0, height: 1.0, isHidden: false)
        //线条相关属性
        open var line = LEYClipboardView.Line()
        //边
        open var side = LEYApp.Rect.Side.unspefified
        //框选区域的frame
        public let frame = LEYApp.Wrapped<Bool, CGRect> { (_, __sender) in
    ...
    open class WrappedView : LEYView {
        ...
        public let minXView = LEYCView<UIView>(frame: CGRect.zero)
        public let maxXView = LEYCView<UIView>(frame: CGRect.zero)
        public let minYView = LEYCView<UIView>(frame: CGRect.zero)
        public let maxYView = LEYCView<UIView>(frame: CGRect.zero)
        
        open override func setupSubviews() {
            self.minXTHSView.backgroundColor = LEYApp.mainColor
            self.minXTHSView.isUserInteractionEnabled = false
            self.addSubview(self.minXTHSView)
            
            self.minXBHSView.backgroundColor = LEYApp.mainColor
            self.minXBHSView.isUserInteractionEnabled = false
            self.addSubview(self.minXBHSView)
            
            self.maxXTHSView.backgroundColor = LEYApp.mainColor
            self.maxXTHSView.isUserInteractionEnabled = false
            self.addSubview(self.maxXTHSView)
            
            self.maxXBHSView.backgroundColor = LEYApp.mainColor
            self.maxXBHSView.isUserInteractionEnabled = false
            self.addSubview(self.maxXBHSView)
            
            self.minYLHSView.backgroundColor = LEYApp.mainColor
            self.minYLHSView.isUserInteractionEnabled = false
            self.addSubview(self.minYLHSView)
            
            self.minYRHSView.backgroundColor = LEYApp.mainColor
            self.minYRHSView.isUserInteractionEnabled = false
            self.addSubview(self.minYRHSView)
            
            self.maxYLHSView.backgroundColor = LEYApp.mainColor
            self.maxYLHSView.isUserInteractionEnabled = false
            self.addSubview(self.maxYLHSView)
            
            self.maxYRHSView.backgroundColor = LEYApp.mainColor
    ...
    public let wrapped = LEYClipboardView.Wrapped()
    public let wrappedView = LEYClipboardView.WrappedView(frame:CGRect.zero)
    public let panRecognizer = UIPanGestureRecognizer()
    public let point = LEYApp.Wrapped<UIGestureRecognizer.State, CGPoint>{ (_, __sender) in
    ...
    open override func updateSubviews(_ action: String, _ value: Any?) {
        ...
        if self.wrapped.clip.isHidden == false {
            ...
            let pfsValue = LEYApp.Rect(completion:nil)
